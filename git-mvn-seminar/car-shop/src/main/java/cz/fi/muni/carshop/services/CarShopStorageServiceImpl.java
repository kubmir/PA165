...
public class CarShopStorageServiceImpl implements CarShopStorageService {
	...
	public void addCarToStorage(Car car) {
            if(car.getPrice() < 0) {
                throw new IllegalArgumentException("Price of car smaller than 0.");
            }
        ...
        public void sellCar(Car car) throws RequestedCarNotFoundException {
            List<Car> tmp = CarShopStorage.getInstancce().getCars().get(car.getType());
            boolean isInStorage = tmp.contains(car);
